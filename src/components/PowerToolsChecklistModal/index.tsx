...
import {
  ...
  DialogTitle, Divider, FormControlLabel, Checkbox,
  IconButton, MenuItem, TextField, Typography, useMediaQuery,
} from "@mui/material"
    ...
    >
      ...
      <DialogContent sx={{ px: 3, py: 2.5, overflowY: "auto", flexGrow: 1 }}>
        <Box display="flex" flexDirection="column" gap={2.5}>
          ...
          >
            <Typography sx={{ fontWeight: 700, fontSize: "0.88rem", letterSpacing: "0.02em", color: "#f0f0f0" }}>
              {tool?.name ?? "—"}
            </Typography>
            <Typography variant="caption" sx={{ color: "rgba(255,255,255,0.4)" }}>
              In Service: {tool?.total_in_service ?? 0} / {tool?.total_quantity ?? 0}
            </Typography>
          </Box>
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, running: e.target.checked }))}
                size="small"
                sx={{ color: "rgba(255,255,255,0.3)", "&.Mui-checked": { color: theme.palette.secondary.main } }}
              />
            }
            label={<Typography sx={{ fontSize: "0.875rem", color: "#e8e8e8" }}>Running</Typography>}
          />

          {/* Fuel level — unified LEVEL_OPTIONS */}
          <TextField
            select
            label="Fuel Level"
